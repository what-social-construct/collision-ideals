...
| \(D_\alpha\simeq_LN\) | residual-factor identification in the nonnormal cubic branch | input `residualEquiv` to `cubicGenericCollisionEquivResidualField` and the cubic witness | Parameterized. The manuscript proves this from nonnormality; Lean does not yet. |
...
| Keller generic geometry | \(F\) is étale and generically finite; \(L/K\) is finite separable | `KellerEtaleBridge F` and `KellerFlatBridge F` in `General.Keller.Interfaces` | Parameterized. These are theorem-target propositions, not proofs from `IsKeller`; the cubic API still assumes separability and generic-source surjectivity explicitly. |
...
| \(\operatorname{Gal}(N/K)\simeq S_3\) | Galois group of the nonnormal cubic normal closure | `NormalClosureData.galoisGroupEquivPermFinThree`; packaged by `CubicS3CollisionWitness` | Exact after degree three, normal-closure data, and explicit \(H\ne1\). |
...
| P0 | \(\operatorname{Spec}D_\alpha\) connected iff \(D_\alpha\) is a field iff \(h_\alpha\) is irreducible iff \(L/K\) is nonnormal | Missing | Formalize the separable quadratic split/field dichotomy and the cubic normality argument. |
| P0 | In the nonnormal branch, \(D_\alpha\simeq_LN\), and the fixing subgroup \(H\) is nontrivial | Parameterized by `residualEquiv` and `hnontrivial` | Derive both inputs from the preceding cubic-branch theorem, then simplify `CubicS3CollisionWitness` consumers. |
| P0 | Classical Galois rigidity: Keller plus normal \(L/K\) implies automorphism and \(L=K\) | Missing | Add a shared General Keller/Galois interface or formal proof; it is not cubic-owned and is also relevant to Paper II. |
...
| P1 | Residual product data imply \(\operatorname{Obs}(F)\ne0\) and \(I_R\subsetneq I_\Delta\) | Exact when `residualEquiv` is supplied | After the P0 cubic-branch theorem, add a wrapper requiring only generic degree three. |
