...
/-!
# Galois collision cocones for polynomial maps

This file constructs, in arbitrary dimension, the collision cocone attached
...
Dimension-specific consequences, such as factoring a moved planar cocone
...
/-- The dimension-independent Galois collision cocone associated to the
ordered pair of conjugate sheets `(g, σg)`. -/
noncomputable def polynomialGaloisCollisionCocone
    (D : PolynomialNormalClosureData F N)
    (g σ : D.galoisGroup) :
    letI : Algebra (PolynomialImageAlgebra F) N :=
      polynomialNormalExtensionBaseAlgebra (F := F) (N := N)
...
@[simp]
theorem polynomialGaloisCollisionCocone_left
    (D : PolynomialNormalClosureData F N)
    (g σ : D.galoisGroup) :
    letI : Algebra (PolynomialImageAlgebra F) N :=
      polynomialNormalExtensionBaseAlgebra (F := F) (N := N)
    letI : Algebra k N :=
      polynomialNormalExtensionGroundAlgebra (F := F) (N := N)
    (polynomialGaloisCollisionCocone D g σ).left =
      polynomialConjugateSourceMap D g := by
  rfl

@[simp]
theorem polynomialGaloisCollisionCocone_right
    (D : PolynomialNormalClosureData F N)
    (g σ : D.galoisGroup) :
    letI : Algebra (PolynomialImageAlgebra F) N :=
      polynomialNormalExtensionBaseAlgebra (F := F) (N := N)
    letI : Algebra k N :=
      polynomialNormalExtensionGroundAlgebra (F := F) (N := N)
    (polynomialGaloisCollisionCocone D g σ).right =
      polynomialConjugateSourceMap D (σ * g) := by
  rfl

/-- The group-theoretic moved-sheet condition directly gives distinct source
maps in the Galois collision cocone. -/
theorem polynomialGaloisCollisionCocone_left_ne_right_of_not_mem
    (D : PolynomialNormalClosureData F N)
    (g σ : D.galoisGroup)
    (hσ : σ ∉ D.intermediateFixingSubgroup.map
      (MulAut.conj g).toMonoidHom) :
    ...
    letI : Algebra k N :=
      polynomialNormalExtensionGroundAlgebra (F := F) (N := N)
    (polynomialGaloisCollisionCocone D g σ).left ≠
      (polynomialGaloisCollisionCocone D g σ).right := by
  letI : Algebra (PolynomialImageAlgebra F) N :=
    polynomialNormalExtensionBaseAlgebra (F := F) (N := N)
  letI : Algebra k N :=
    polynomialNormalExtensionGroundAlgebra (F := F) (N := N)
  simpa only [polynomialGaloisCollisionCocone_left,
    polynomialGaloisCollisionCocone_right] using
