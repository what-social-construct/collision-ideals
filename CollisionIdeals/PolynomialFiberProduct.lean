...
def collisionTensorEquiv (F : κ → SourceRing R ι) :
  ...
  let tensorCocone : CollisionCocone F TP :=
    { left :=
  ...
  · apply AlgHom.ext
    ...
    | tmul a b =>
        have hleft :
          ...
          exact AlgHom.congr_fun (collisionLift_comp_left tensorCocone) a
        have hright :
            forward (collisionRight F b) = tensorCocone.right b := by
          exact AlgHom.congr_fun (collisionLift_comp_right tensorCocone) b
        change
          forward
  ...
  · apply collisionHom_ext
    · apply MvPolynomial.algHom_ext
      intro i
      have hforward :
          forward (collisionLeft F (X i)) = tensorCocone.left (X i) := by
        exact
          AlgHom.congr_fun (collisionLift_comp_left tensorCocone) (X i)
    ...
    · apply MvPolynomial.algHom_ext
      intro i
      have hforward :
          forward (collisionRight F (X i)) = tensorCocone.right (X i) := by
        exact
          AlgHom.congr_fun (collisionLift_comp_right tensorCocone) (X i)
