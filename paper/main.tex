...
For \(E\in\Ram^{(1)}(Z/Y)\), put
\[
  \mathfrak j_E^{\mathrm{mov}}
  :=
  \sum_{\substack{g\in G\\I_E\nsubseteq gHg^{-1}}}\mathfrak j_g.
\]
Then
\[
  \exists g\in G,\quad
...
\[
  ...
  \end{gathered}
\]
The next theorem records the remaining named objects as equivalent
translations of this spine.
...
\subsubsection{Fixed--moving boundary principal parts}

For a normal Noetherian integral surface \(W=\Spec R\) with dense open
\(U=W\setminus V(\mathfrak j)\), define its boundary principal-parts module
by
\[
  \mathcal P_{\mathfrak j}(R):=H^1_{\mathfrak j}(R).
\]
The open-complement sequence identifies it with rational functions regular
on \(U\) modulo those extending to \(W\).  At a height-one prime,
\[
  \mathcal P_{\mathfrak j}(R)_{\mathfrak p}
  \cong
  \begin{cases}
    0,&\mathfrak j\nsubseteq\mathfrak p,\\[2pt]
    \Frac(R_{\mathfrak p})/R_{\mathfrak p},
      &\mathfrak j\subseteq\mathfrak p.
  \end{cases}
\]
Indeed, localization reduces to the one-element \v{C}ech complex of the DVR
\(R_{\mathfrak p}\).  Put \(T:=A_N\).  For a prime-order subgroup
\(C\leq G\), set
\[
  \begin{aligned}
    U_C^{\mathrm{mov}}
      &:=Z\setminus V(\mathfrak j_C^{\mathrm{mov}})
        =\bigcup_{C\nsubseteq gHg^{-1}}U_g,\\
    \mathcal P_C^{\mathrm{mov}}
      &:=H^1_{\mathfrak j_C^{\mathrm{mov}}}(T),
    &
    \mathcal P_C^{\mathrm{fix,mov}}
      &:=H^0_{\mathfrak a_C}
          \bigl(\mathcal P_C^{\mathrm{mov}}\bigr).
  \end{aligned}
\]
We call the last module the \emph{fixed--moving boundary contribution}.
It uses support on the scheme-theoretic fixed locus; it does not take
\(C\)-invariants.  Since \(\core_G(H)=1\), a nontrivial \(C\) moves at
least one conjugate sheet.  The domain property then gives
\(H^0_{\mathfrak j_C^{\mathrm{mov}}}(T)=0\).  The spectral sequence for
successive supports, using
\(\Gamma_{\mathfrak a_C}\Gamma_{\mathfrak j_C^{\mathrm{mov}}}
=\Gamma_{\mathfrak a_C+\mathfrak j_C^{\mathrm{mov}}}\), therefore gives
the canonical identification
\[
  \mathcal P_C^{\mathrm{fix,mov}}
  \cong
  H^1_{\mathfrak a_C+\mathfrak j_C^{\mathrm{mov}}}(T).
\]
For a height-one prime \(\mathfrak p_E\subset T\), the same DVR calculation
gives
\[
  \left(\mathcal P_C^{\mathrm{fix,mov}}\right)_{\mathfrak p_E}
  \cong
  \begin{cases}
    \Frac(T_{\mathfrak p_E})/T_{\mathfrak p_E},
      &\mathfrak a_C+\mathfrak j_C^{\mathrm{mov}}
         \subseteq\mathfrak p_E,\\[2pt]
    0,&\text{otherwise}.
  \end{cases}
\]
The first case says exactly that \(C\leq I_E\) and that \(E\) belongs to
every boundary \(D_g\) whose sheet is moved by \(C\).  Thus failure of
\(\operatorname{Sep}_2(F)\) produces precisely an unbounded height-one
localization of \(\mathcal P_C^{\mathrm{fix,mov}}\).  Conversely, normality
makes \(T\) an (S_2\) surface, so when the displayed support has no
height-one point its grade is at least two and this first local cohomology
vanishes.  Hence
\[
  \operatorname{Sep}_2(F)
  \quad\Longleftrightarrow\quad
  \mathcal P_C^{\mathrm{fix,mov}}=0
  \quad\text{for every prime-order }C\leq G.
\]

The pole filtration records the same obstruction equivariantly.  In the
first case let \(\chi_E\) denote the tame character of \(C\) on the
cotangent line of the DVR (T_{\mathfrak p_E}\).  For
\[
  F_m:=\pi_E^{-m}T_{\mathfrak p_E}/T_{\mathfrak p_E},
  \qquad m\geq0,
\]
one has
\[
  \operatorname{gr}_m^F
    \left(\mathcal P_C^{\mathrm{fix,mov}}\right)_{\mathfrak p_E}
  \cong \kappa(E)\otimes\chi_E^{-m}.
\]
If \(\sigma\) generates \(C\), \(\chi_E(\sigma)=\zeta\), and
\(\ell=|C|\), its formal graded pole-character is
\[
  \sum_{m\geq1}\zeta^{-m}q^m
  =\frac{\zeta^{-1}q}{1-\zeta^{-1}q},
  \qquad
  \text{with invariant subseries }
  \frac{q^\ell}{1-q^\ell}.
\]
A bounded codifferent stage instead has a polynomial character.  This is
formally analogous to fixed-point and tangent-cone character expansions
\cite{SardoInfirri1995}, but no global Lefschetz formula is being asserted:
local rational tails can cancel in a virtual character, while the present
problem requires an actual module-theoretic pole bound.

\subsubsection{Trace-dual target and secant transport}

There is nevertheless a canonical finite bounded-pole lattice.  Write
\[
  T^\dagger
  :=\{z\in N:\operatorname{Tr}_{N/K}(zT)\subseteq B\}
  \cong \operatorname{Hom}_B(T,B).
\]
Finite flatness and generic separability identify \(T^\dagger\) with the
finite \(B\)-module \(\operatorname{Hom}_B(T,B)\) through the trace pairing.
Because \(B\) is normal, the trace of every element integral over \(B\) lies
in \(B\), so \(T\subseteq T^\dagger\).  Consequently
\[
  Q_{\mathrm{tr}}:=T^\dagger/T
\]
is finite.  If \(\mathfrak q_E=\mathfrak p_E\cap B\), then after localizing
the DVR extension over \(B_{\mathfrak q_E}\), separating its unramified
part, and choosing a uniformizer \(\pi_E\) of \(T_{\mathfrak p_E}\), the
characteristic-zero tame formula gives the bounded codifferent stage
\[
  (Q_{\mathrm{tr}})_{\mathfrak p_E}
  \cong
  \pi_E^{-(e_E-1)}T_{\mathfrak p_E}/T_{\mathfrak p_E}.
\]
Here ``codifferent'' means the trace-dual lattice.  Its inverse agrees with
the K\"ahler different after height-one localization; no global invertibility
or global identification with \(\operatorname{Fitt}_0\Omega_{T/B}\) is used
\cite{StacksDedekindDifferent,StacksFiniteFlatDifferent}.

Write
\(\omega_Z:=\operatorname{Hom}_B(T,\omega_Y)\) for the finite-duality
canonical module.  Trivializing \(\omega_Y\) by \(dP\wedge dQ\) identifies
\(\omega_Z\) with
\(T^\dagger\cong\operatorname{Hom}_B(T,B)\)
\cite{StacksQuasiFiniteDualizing}.  If
\[
  \eta_F:=c^{-1}dP\wedge dQ,
\]
then the Keller identity restricts \(\eta_F\) to
\(d(gx_1)\wedge d(gx_2)\) on the sheet \(X_g\).  Under the trace
identification,
\[
  T\eta_F\subseteq\omega_Z,
  \qquad
  Q_F^{\mathrm{sec}}
  :=\omega_Z/T\eta_F
  \cong T^\dagger/T=Q_{\mathrm{tr}}.
\]
Thus the ``secant canonical-form target'' is not a second finite module: it
is the same codifferent quotient described geometrically.  At a tame
ramified divisor one has
\(\eta_F=u\pi_E^{e_E-1}\omega_E\) for a unit \(u\) and a local generator
\(\omega_E\) of \(\omega_Z\), recovering the bounded-pole formula above.
The Hilbert--Burch trivialization of
Proposition~\ref{prop:secant-hilbert-burch} supplies a natural generic-sheet
presentation, but identifying its chosen generator with \(\eta_F\) is a
separate comparison and is not needed for the finite target itself.

The natural inclusion runs from this bounded stage into principal parts,
...
\[
  ...
  \Frac(T_{\mathfrak p_E})/T_{\mathfrak p_E}.
\]
Indeed, every \(T_{\mathfrak p_E}\)-linear map from the divisible module
\(\Frac(T_{\mathfrak p_E})/T_{\mathfrak p_E}\) to a finite
\(T_{\mathfrak p_E}\)-module is zero.  Thus finite flatness does not supply
an injection of the fixed--moving principal-parts module into
\(Q_{\mathrm{tr}}\); such an injection would already exclude a divisorial
fixed--moving boundary component.

This bounded stage nevertheless gives a canonical diagnostic.  The
open-complement exact sequence embeds
\(\mathcal P_C^{\mathrm{mov}}\cong
\Gamma(U_C^{\mathrm{mov}},\mathcal O_Z)/T\) into \(N/T\); hence it embeds
\(\mathcal P_C^{\mathrm{fix,mov}}\), while
\(T\subseteq T^\dagger\subseteq N\) embeds \(Q_{\mathrm{tr}}\) in the same
module.  Define the trace-comparison ideal
\[
  \mathfrak c_C^{\mathrm{fm,tr}}
  :=
  \{s\in T:
      s\mathcal P_C^{\mathrm{fix,mov}}\subseteq Q_{\mathrm{tr}}\}.
\]
It is important to impose this condition only on the
fixed--moving contribution.  Imposing it on the whole ring
\(\Gamma(U_C^{\mathrm{mov}},\mathcal O_Z)\) would also detect unrelated
boundary divisors and can force the ideal to be zero for reasons stronger
than the hidden-inertia problem.

Multiplication defines a canonical map
\[
  \Phi_C:\mathcal P_C^{\mathrm{fix,mov}}
  \longrightarrow
  \operatorname{Hom}_T
    (\mathfrak c_C^{\mathrm{fm,tr}},Q_{\mathrm{tr}}),
  \qquad
  \Phi_C(\xi)(s)=s\xi.
\]
The target is finite: \(T\) is Noetherian, so
\(\mathfrak c_C^{\mathrm{fm,tr}}\) is finite, and both
\(Q_{\mathrm{tr}}\) and the displayed Hom module are finite.  More sharply,
if a height-one prime contains
\(\mathfrak a_C+\mathfrak j_C^{\mathrm{mov}}\), then
\[
  \mathfrak c_C^{\mathrm{fm,tr}}=0.
\]
Indeed, let \(0\ne s\in\mathfrak c_C^{\mathrm{fm,tr}}\) and localize at
\(\mathfrak p_E\).  Multiplication by the nonzero element \(s\) on the
divisible module
\(\Frac(T_{\mathfrak p_E})/T_{\mathfrak p_E}\) is surjective.  Its image
therefore still has arbitrarily deep poles and cannot lie in the bounded
finite module \((Q_{\mathrm{tr}})_{\mathfrak p_E}\), a contradiction.
Thus the exact prospective target is simply
\[
  \mathfrak c_C^{\mathrm{fm,tr}}\ne0
  \qquad\text{for every prime-order }C\leq G.
\]
More concretely, one may construct a nonzero secant/frame ideal
\[
  0\ne\mathfrak s_C^{\mathrm{sec}}
  \subseteq\mathfrak c_C^{\mathrm{fm,tr}}.
\]
No separate condition
\(\mathfrak s_C^{\mathrm{sec}}\nsubseteq\mathfrak p_E\) is needed: every
nonzero element of the domain \(T\) remains nonzero in
\(T_{\mathfrak p_E}\), and multiplication by it on the divisible quotient
is already surjective.  This transporter construction
packages the desired uniform pole bound; trace duality alone gives no reason
for the transporter to be nonzero.

The fact that \(A_N\) or a boundary quotient is
finite over its base does not give the opposite conclusion.  For example,
let
\[
  B_0=\C[a,t]\subseteq R=\C[s,t],
  \qquad a=s^e.
\]
Then \(R\) is finite free over \(B_0\), and the boundary curve
\(R/(s)\cong B_0/(a)\) is finite over its image, whereas
\[
  H^1_{(s)}(R)=R_s/R=\bigcup_{m\geq1}s^{-m}R/R
\]
is not finitely generated.  What is missing is a uniform transverse pole
bound.  The precise target is a Keller-specific theorem showing that the
fixed--moving boundary classes lie in one fixed fractional stage.  Writing
\[
  \mathfrak d_N:=\operatorname{Fitt}^T_0\Omega_{T/B},
\]
define
\[
  Q_{\mathfrak d}^{(m)}
  :=(T:_N\mathfrak d_N^m)/T
  =\{z\in N:\mathfrak d_N^m z\subseteq T\}/T,
\]
which avoids treating the K\"ahler different as a globally invertible ideal.
After localization at \(\mathfrak p_E\), this becomes the bounded stage
\[
  \left(Q_{\mathfrak d}^{(m)}\right)_{\mathfrak p_E}
  \cong
  \pi_E^{-m(e_E-1)}T_{\mathfrak p_E}/T_{\mathfrak p_E}.
\]
Alternatively, one may ask for a uniform power of
\(\mathfrak a_C+\mathfrak j_C^{\mathrm{mov}}\) to annihilate
\(\mathcal P_C^{\mathrm{fix,mov}}\).  Either statement is incompatible with
the DVR pole tower, but neither follows from finite flatness and neither is
constructed here
\cite{StacksLocalCohomology,StacksLocalCohomologyFiniteness}.  Likewise, the
secant determinant detects marked versus moving generic sheets, but it
vanishes on a moving sheet and by itself gives no nonzero parameter that
bounds boundary poles.

The inverse-Jacobian frame supplies plausible input for this last detection
problem.  Writing \(c=\det JF\), the polynomial derivations
\[
  \partial_P=c^{-1}
    \left(Q_{x_2}\frac{\partial}{\partial x_1}
          -Q_{x_1}\frac{\partial}{\partial x_2}\right),
  \qquad
  \partial_Q=c^{-1}
    \left(-P_{x_2}\frac{\partial}{\partial x_1}
          +P_{x_1}\frac{\partial}{\partial x_2}\right)
\]
restrict to the coordinate derivations on \(K=\C(P,Q)\).  Their unique
extensions to the finite separable normal closure commute with \(G\), hence
preserve every conjugate affine ring \(gA\).  In the standard tame
height-one model after a suitable completed Kummer base change (including a
root of the unit), \(u=t^e\), and differentiating gives
\(\partial t/\partial u=(e t^{e-1})^{-1}\), the expected
inverse-different denominator.  Intrinsically, only its valuation
\(-(e-1)\) is used.  A prospective secant--trace landing theorem
would use
the evaluated full secant matrix or its adjugate, together with this frame,
to produce a nonzero \(\mathfrak s_C^{\mathrm{sec}}\).  Neither trace landing
nor nonvanishing of such an ideal is asserted here.
...
\midrule
\(\operatorname{Sep}_2\), \(\operatorname{Cov}_2\), and
\(\operatorname{PRR}\) & Ideal, valuation, and differential descriptions
of absence of divisorial ramification & Equivalent for each planar Keller
map; universal validity is equivalent to \(JC(2)\) \\
\(\operatorname{PBC}\) & Global boundary/properness description &
Equivalent to automorphy for each map; not known universally \\
Torsion of \([R_{\bar\nu}]\) & Sufficient divisor-class mechanism &
Established for normal/Galois \(L/K\), hence for generic degree two \\
Vanishing or uniform annihilation of
\(\mathcal P_C^{\mathrm{fix,mov}}\) & Fixed--moving principal-parts
description of boundary separation & A divisorial fixed--moving locus gives
the unbounded DVR pole tower; uniform control is not derived from Keller data \\
Fixed--moving secant--trace boundedness & Possible boundary-compatible
extension of generic sheet isolation & Secant coevaluation and the
trace-dual/canonical-form target are explicit; no secant/frame theorem
supplies a nonzero multiplier landing all relevant classes in it \\
\bottomrule
\end{tabular}
\end{center}

Thus the purity argument is the unique endgame.  The divisor-class and
principal-parts constructions are possible ways of establishing its
height-one premise, not parallel conclusions that the reader must combine.

\begin{leanfiles}
The formal planar implication chains are collected in
\leanpath{CollisionIdeals/Planar.lean}.  The development takes the
following established results as explicit inputs: planar
Ax--Grothendieck, branch purity, and finite-\'etale rigidity of
\(\A^2_{\C}\).  It also takes a supplied
\leanpath{PlanarKellerCollisionModel F}.  The positive
\leanpath{PlanarHiddenInertia} predicate and the implication from
\leanpath{PlanarConjugateCoverage} to
\leanpath{PlanarNoHiddenInertia} are formalized; constructing the model and
proving coverage remain separate formalization tasks.  The fixed-locus
ideal \leanpath{fixedLocusIdeal}, the moving-boundary sum
\leanpath{movingBoundaryIdeal}, and the height-one condition
\leanpath{BoundaryIdealData.PlanarBoundarySeparation} are formalized in
\leanpath{CollisionIdeals/Planar/BoundarySeparation.lean}.  The Lean
predicate quantifies over all nontrivial subgroups; the
manuscript's prime-order formulation is its equivalent Cauchy reduction.
The theorem
\leanpath{BoundaryIdealData.ramifiedCodimensionOnePoint_isEmpty} connects
that condition to the existing no-ramification endgame.  The pulled-back
open \leanpath{pulledBackConjugateAffineOpen}, its closed complement
\leanpath{pulledBackConjugateBoundary}, and its vanishing ideal
\leanpath{pulledBackConjugateBoundaryIdeal} are constructed canonically;
\leanpath{pulledBackConjugateBoundaryIdeal_le_ramifiedPrime} formalizes the
valuation-theoretic containment at a ramified divisor.  The exact pointwise
coverage assertion and the universal claims
\(\operatorname{PBC}(F)\) or \(\operatorname{PRR}(F)\) remain hypotheses at
the present Lean boundary, not conclusions of the formalization.  The local-cohomology
target is connected to the downstream endgame by the
explicit input \leanpath{BoundaryCoherenceBridge} and theorem
\leanpath{planarVanishing_of_boundaryCoherence}; the bridge itself is not
proved.  Likewise, \leanpath{RamificationRigidityBridge} isolates the
unformalized finite-length support step in the divisorial route.  The stable
local-cohomology object \leanpath{BoundaryPrincipalParts} is defined in
\leanpath{CollisionIdeals/BoundaryPrincipalParts.lean}.  The general
combined-support and uniform-annihilator objects
\leanpath{CombinedSupportPrincipalParts} and
\leanpath{HasUniformCombinedSupportAnnihilator} are kept in
\leanpath{CollisionIdeals/Planar/Research/PrincipalPartsStrategy.lean}.
Their canonical fixed--moving specialization
\leanpath{FixedMovingBoundaryPrincipalParts}, its support ideal
\leanpath{fixedMovingBoundarySupportIdeal}, and its exact equivalence with
the existing height-one separation predicate are formalized in
\leanpath{CollisionIdeals/Planar/Research/FixedMovingBoundaryPrincipalParts.lean}.
The successive-support isomorphism, DVR localization, and graded
pole-character used in the manuscript are not yet formalized.  The abstract
nonfiniteness mechanism for unbounded locally power-torsion principal parts
is proved in
\leanpath{CollisionIdeals/Planar/Research/UnboundedPrincipalParts.lean}; finite
comparison control and the resulting direct contradiction are formalized in
\leanpath{CollisionIdeals/Planar/Research/FinitePrincipalPartsControl.lean}.  The
evaluation of the planar secant relation on an arbitrary pair of collision
sheets, including its vanishing when a coordinate is moved, is formalized in
\leanpath{CollisionIdeals/Planar/ConjugateSecantEvaluation.lean}.  The
prospective modules are collected under the opt-in umbrella
\leanpath{CollisionIdeals/Planar/Research.lean}, rather than the stable
\leanpath{CollisionIdeals/Planar.lean} import spine.  The
generic trace-integral submodule
\leanpath{TraceIntegralSubmodule}, the transporter
\leanpath{boundedStageComparisonIdeal}, and its canonical multiplication map
\leanpath{boundedStageComparisonMap} are also formalized abstractly in the
research layer.  Their specialization to the finite normalization---including
the trace-dual/Hom identification, the open-complement embeddings, the
height-one DVR calculation, and especially a nonzero secant multiplier---
remains to be constructed.  The boundary-coherence implication and the
finite-length differentials criterion likewise remain separate formalization
tasks.  The properness and
divisor-class criteria above are likewise not presently part of the Lean
development.
\end{leanfiles}
