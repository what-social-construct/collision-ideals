...
\hypersetup{
  ...
  pdfauthor={Chloe van der Vlugt; Codex 5.6 Sol},
  colorlinks=true,
...
\]
The inclusion \(I_R\subseteq I_\Delta\) defines the obstruction ideal
\(\Obs(F):=I_\Delta/I_R\subseteq C_F:=S/I_R\), and
\[
  \Obs(F)=0
...
\leanpath{PlanarKellerCollisionModel F} and the no-hidden-inertia
hypothesis; constructing the model and proving that hypothesis remain
separate formalization tasks.
\end{leanfiles}
