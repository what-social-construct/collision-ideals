...
For a Noetherian domain \(R\) and a nonzero ideal \(\mathfrak j\), define
the boundary principal-parts module by
\[
  \mathcal P_{\mathfrak j}(R):=H^1_{\mathfrak j}(R).
\]
For \(W=\Spec R\) and the dense open
\(U=W\setminus V(\mathfrak j)\), the open-complement sequence identifies it
with rational functions regular on \(U\) modulo those extending to \(W\).
If \(R\) is normal and \(\mathfrak p\) has height one, then
\[
...
\[
  ...
  \end{aligned}
\]
We call this the \emph{fixed--moving boundary contribution}.  It is first
local cohomology with support on the scheme-theoretic intersection of the
fixed locus and all moved-sheet boundaries; it does not take
\(C\)-invariants.  For comparison, put
\[
  \mathcal P_C^{\mathrm{mov}}
  :=H^1_{\mathfrak j_C^{\mathrm{mov}}}(T).
\]
Since \(\core_G(H)=1\), a nontrivial \(C\) moves at
...
the canonical successive-support interpretation
\[
  H^0_{\mathfrak a_C}\bigl(\mathcal P_C^{\mathrm{mov}}\bigr)
  \cong \mathcal P_C^{\mathrm{fix,mov}}.
\]
...
bound.  Equivalently, one may ask for a uniform power of
\(\mathfrak a_C+\mathfrak j_C^{\mathrm{mov}}\) to annihilate
\(\mathcal P_C^{\mathrm{fix,mov}}\).  Either statement is incompatible with
the DVR pole tower, but neither follows from finite flatness; the trace-dual
quotient above is the canonical bounded stage used here
...
\leanpath{movingBoundaryIdeal}, the canonical sum
\leanpath{fixedMovingBoundaryIdeal}, and the height-one condition
\leanpath{PlanarBoundarySeparation} are formalized in
...
\leanpath{CollisionIdeals/BoundaryPrincipalParts.lean}.  The canonical
fixed--moving module \leanpath{FixedMovingBoundaryPrincipalParts} and its
specialized \leanpath{HasUniformFixedMovingBoundaryAnnihilator} target are
formalized in
...
\leanpath{TraceIntegralSubmodule} and the transporter
\leanpath{boundedStageComparisonIdeal} are also formalized abstractly in the
