...
For a normal Noetherian surface \(W=\Spec R\) with dense open
...
\(R_{\mathfrak p}\).  For a nontrivial \(I\leq G\), put
\[
  \mathcal P_I^{\mathrm{mov}}
  :=H^1_{\mathfrak j_I^{\mathrm{mov}}}(A_N),
  \qquad
  \mathcal P_I^{\mathrm{ram}}
  :=H^0_{\mathfrak d_N}(\mathcal P_I^{\mathrm{mov}}),
  \qquad
  \mathfrak d_N:=\mathfrak D_{A_N/B}.
\]
Successive supports give
\[
  \mathcal P_I^{\mathrm{ram}}
  \cong
  H^1_{\mathfrak d_N+\mathfrak j_I^{\mathrm{mov}}}(A_N).
\]
If \(E\) is a hidden ramified divisor with \(I=I_E\), then the pulled-back
boundary dictionary and the DVR calculation give
\[
  (\mathcal P_I^{\mathrm{ram}})_{\mathfrak p_E}
  \cong
  \Frac((A_N)_{\mathfrak p_E})/(A_N)_{\mathfrak p_E},
\]
whose pole orders are unbounded; hence
\(\mathcal P_I^{\mathrm{ram}}\) is not finite.  This is the nonfiniteness
half of a prospective contradiction.
...
\[
  ...
  \qquad T:=A_N.
\]
Finite flatness makes \(T^\dagger\) a finite \(B\)-module, and integrality of
traces gives \(T\subseteq T^\dagger\).  Consequently
\[
  Q_{\mathrm{tr}}:=T^\dagger/T
\]
is finite.  At a height-one tame DVR with uniformizer \(\pi_E\) and
ramification index \(e_E\), it is the bounded codifferent stage
\[
...
an injection of the entire ramification-supported principal-parts module into
\(Q_{\mathrm{tr}}\); such an injection would already exclude a hidden
ramified divisor.

This bounded stage nevertheless gives a canonical comparison construction.
View \(Q_{\mathrm{tr}}\) as a submodule of the rational principal-parts
module \(N/T\), and view \(\mathcal P_I^{\mathrm{ram}}\) inside \(N/T\) via
the open-complement description.  Define the trace-comparison ideal
\[
  \mathfrak c_I^{\mathrm{tr}}
  :=
  \{s\in T:s\mathcal P_I^{\mathrm{ram}}\subseteq Q_{\mathrm{tr}}\}.
\]
It is important to impose this condition only on the
ramification-supported classes.  Imposing it on the whole ring
\(\Gamma(U_I^{\mathrm{mov}},\mathcal O_Z)\) would also detect unrelated
unramified boundary divisors and can force the ideal to be zero for reasons
stronger than the hidden-inertia problem.

Multiplication defines a canonical map
\[
  \Phi_I:\mathcal P_I^{\mathrm{ram}}
  \longrightarrow
  \operatorname{Hom}_T
    (\mathfrak c_I^{\mathrm{tr}},Q_{\mathrm{tr}}),
  \qquad
  \Phi_I(\xi)(s)=s\xi.
\]
The target is finite: \(T\) is Noetherian, so
\(\mathfrak c_I^{\mathrm{tr}}\) is finite, and both
\(Q_{\mathrm{tr}}\) and the displayed Hom module are finite.  If \(E\) is
hidden with \(I_E=I\) and
\(\mathfrak c_I^{\mathrm{tr}}\nsubseteq\mathfrak p_E\), then some comparison
multiplier is a unit after localizing at \(E\), so
\((\Phi_I)_{\mathfrak p_E}\) is injective.  This would inject
\(\Frac(T_{\mathfrak p_E})/T_{\mathfrak p_E}\) into a finite module, a
contradiction.  The remaining geometric problem is therefore the precise
height-one detection assertion
\[
  E\text{ hidden with }I_E=I
  \quad\Longrightarrow\quad
  \mathfrak c_I^{\mathrm{tr}}\nsubseteq\mathfrak p_E.
\]
Equivalently, one may construct a finitely generated secant/frame ideal
\(\mathfrak s_I^{\mathrm{sec}}\subseteq\mathfrak c_I^{\mathrm{tr}}\) that is
not contained in any such \(\mathfrak p_E\).
...
relevant boundary classes lie in one fixed codifferent stage
\(\mathfrak d^{-m}/T\), or equivalently that they admit a uniform
different-power annihilator; neither assertion follows from finite flatness
and neither is constructed here.  More explicitly, if some power of
\(\mathfrak d_N+\mathfrak j_I^{\mathrm{mov}}\) uniformly annihilated the
displayed local-cohomology module for every relevant \(I\), local-cohomology
finiteness and the DVR detector would force its vanishing and hence coverage
...
preserve every conjugate affine ring \(gA\).  In the tame height-one model
\(u=t^e\), differentiating gives
\(\partial t/\partial u=(e t^{e-1})^{-1}\), the expected
inverse-different denominator.  A prospective secant--trace landing theorem would use
the evaluated full secant matrix or its adjugate, together with this frame,
to produce \(\mathfrak s_I^{\mathrm{sec}}\).  Neither trace landing nor
height-one detection is asserted here.
...
Finiteness or uniform annihilation of \(\mathcal P_I^{\mathrm{ram}}\) &
Sufficient principal-parts mechanism & Hidden inertia gives the opposite
nonfiniteness; finite control is not derived from the Keller data \\
Secant--trace/codifferent boundedness & Possible source of a fixed bounded
pole stage & The trace dual supplies the stage; no secant/frame theorem forces
the relevant classes into it \\
\bottomrule
\end{tabular}
\end{center}
...
\leanpath{CollisionIdeals/BoundaryPrincipalParts.lean}.  The
ramification-supported objects
\leanpath{RamificationSupportedBoundaryPrincipalParts} and
\leanpath{HasUniformBoundaryAnnihilator} are kept in the prospective module
\leanpath{CollisionIdeals/Planar/PrincipalPartsStrategy.lean}.  The abstract
nonfiniteness mechanism for unbounded locally power-torsion principal parts
is proved in
\leanpath{CollisionIdeals/Planar/UnboundedPrincipalParts.lean}; finite
comparison control and the resulting direct contradiction are formalized in
\leanpath{CollisionIdeals/Planar/FinitePrincipalPartsControl.lean}.  The
...
geometric comparison with sections on an open complement, the height-one DVR
detector, the boundary-coherence implication, the finite-length
differentials criterion, the trace-dual bounded stage, and the construction
of a uniform codifferent bound from the planar Keller data remain separate
formalization tasks.  The properness and
