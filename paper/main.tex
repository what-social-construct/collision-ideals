...
The open-complement sequence and the inclusions
\(T\subseteq T^\dagger\subseteq N\) place both modules in the same ambient
principal-parts module:
\[
  \iota_C\colon\mathcal P_C^{\mathrm{fm}}\lhook\joinrel\longrightarrow N/T,
  \qquad
  \iota_{\mathrm{tr}}\colon Q_{\mathrm{tr}}
    \lhook\joinrel\longrightarrow N/T.
\]
Statements I and II therefore isolate the exact contrast
\[
  \underbrace{\mathcal P_C^{\mathrm{fm}}}
    _{\text{unbounded if a hidden divisor exists}}
  \qquad\text{and}\qquad
  \underbrace{Q_F^{\omega}\cong Q_{\mathrm{tr}}}
    _{\text{finite and bounded}}.
\]

\paragraph{Missing morphism: Secant--Frame Trace Landing.}
The frame used below is the one constructed in
Proposition~\ref{prop:keller-inverse-jacobian-frame}; in particular, no
preservation of the integral order \(T\) is being assumed.

For each prime-order \(C\), construct explicitly from the full secant
matrix, its adjugate, and the inverse-Jacobian frame a nonzero element
\(s_C^{\mathrm{sec}}\in T\) and a \(T\)-linear morphism
\[
  \lambda_C\colon\mathcal P_C^{\mathrm{fm}}\longrightarrow Q_{\mathrm{tr}}
\]
compatible with multiplication in the common ambient module:
\[
  \iota_{\mathrm{tr}}\circ\lambda_C
  =m_{s_C^{\mathrm{sec}}}\circ\iota_C
  \qquad\text{in }N/T.
\]
Equivalently,
\[
  s_C^{\mathrm{sec}}\mathcal P_C^{\mathrm{fm}}
  \subseteq Q_{\mathrm{tr}}\subseteq N/T.
\]
The map \(\lambda_C\) is not required to be injective.  If a height-one
prime contained \(\mathfrak J_C\), Statement I would identify the localized
source with \(\Frac(T_{\mathfrak p})/T_{\mathfrak p}\).  Multiplication by
any nonzero element of the DVR on this quotient is surjective, whereas
Statement II places the localized target in a proper bounded pole stage.
The compatibility equation is therefore impossible, eliminating that
height-one prime.  No extra condition
\(s_C^{\mathrm{sec}}\notin\mathfrak p\) is needed.

It is essential that \(s_C^{\mathrm{sec}}\) be prescribed by the secant and
frame data.  Bare existence of some nonzero multiplier merely repackages
boundary separation: if Statement I gives
\(\mathcal P_C^{\mathrm{fm}}=0\), one may take \(s_C=1\).  Trace duality
proves Statement II, and generic secant coevaluation isolates the sheets,
but neither supplies the displayed landing morphism.  Constructing it by a
concrete trace identity is the remaining Keller-specific theorem, not a
consequence of finite flatness.

Thus the purity argument remains the unique endgame.  Statements I and II
identify its unbounded source and finite target; the single missing
secant--trace morphism is the proposed way to establish its height-one
premise.

\begin{leanfiles}
The stable API formalizes the canonical ideals
\leanpath{fixedLocusIdeal}, \leanpath{movingBoundaryIdeal}, and
\leanpath{fixedMovingBoundaryIdeal}, together with
\leanpath{PlanarBoundarySeparation}, the pulled-back conjugate boundaries,
their valuation-theoretic containment at ramified divisors, and the ensuing
purity endgame.  It takes planar Ax--Grothendieck, branch purity, finite-
\'etale rigidity, and a supplied \leanpath{PlanarKellerCollisionModel F} as
explicit interfaces.  The Lean separation predicate currently quantifies
over every nontrivial subgroup; formalizing its prime-order Cauchy reduction
is a remaining routine API task.

The general object \leanpath{BoundaryPrincipalParts} and its canonical
specialization \leanpath{FixedMovingBoundaryPrincipalParts} are defined.
The local-cohomology localization, DVR computation, and \(S_2\)-vanishing
used in Statement~I are not yet formalized.  The research layer defines the
generic \leanpath{TraceIntegralSubmodule} and
\leanpath{boundedStageComparisonIdeal}; specializing them to prove the
trace-dual/Hom and local inverse-different assertions of Statement~II is
also unfinished.  Conjugate-sheet secant evaluation is formalized in
\leanpath{CollisionIdeals/Planar/ConjugateSecantEvaluation.lean}.  The common
embeddings in \(N/T\), and especially the nonzero Secant--Frame Trace
Landing morphism, remain to be constructed.  These prospective objects are
kept behind \leanpath{CollisionIdeals/Planar/Research.lean}, outside the
stable import spine.
\end{leanfiles}
