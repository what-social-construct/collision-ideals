...
For \(g\in G\), evaluate the full secant matrix and its adjugate on the
\(g\)-conjugate sheet:
\[
  M_{F,g}:=M_F(x,g(x))\in M_2(N),
  \qquad
  A_{F,g}:=\operatorname{adj}(M_{F,g}).
\]
...
\[
  ...
  :=T+\sum_{\substack{g\in G\\C\nsubseteq gHg^{-1}}}
       \ \sum_{\xi\in\Xi_{g,r}}T\xi\subseteq N,
  \qquad
  \mathfrak d_{C,r}^{\mathrm{sf}}
  :=\{s\in T:s\Lambda_{C,r}^{\mathrm{sf}}\subseteq T\}.
\]
Indexing over all \(g\in G\) avoids any choice of coset representatives; the
family is still finite.  The case \(r=1\) is the first-jet candidate obtained
from the secant matrix, its adjugate, and the inverse-Jacobian frame.
...
\paragraph{The monogenic secant--trace order.}
The finite separable extension \(N/K\) has a primitive element.  Clearing one
...
\(\mathfrak f_{\mathrm{sec}}\) is the conductor of the finite
normalization \(R_{\mathrm{sec}}\subseteq T\)
\cite{StacksPrimitiveElement,StacksHypersurfaceDifferent}.
...
The nonzero-denominator proposition then supplies
\(0\neq s_C^{\mathrm{sf}}\in\mathfrak d_{C,1}^{\mathrm{sf}}\), and that
identity defines the required map explicitly by
\[
...
No identity proved here yet shows that first jets suffice; \(r=1\) is the
first sharply typed candidate.
...
Mathlib's \leanpath{Submodule.traceDual} on the unit order.  Specializing
that equality to prove the trace-dual/Hom finiteness and local
inverse-different assertions of Statement~II is unfinished.  Conjugate-sheet
secant evaluation is formalized in
\leanpath{CollisionIdeals/Planar/ConjugateSecantEvaluation.lean}; the
choice-free telescoping coefficients and polynomial Keller frame are in
\leanpath{Planar.ExplicitSecant} and \leanpath{Planar.KellerFrame}.  The
research theorem \leanpath{finiteCoefficientDenominatorIdeal_ne_bot}
formalizes finite denominator clearing, but the actual conjugate evaluated
family is missing.  The opt-in module
\leanpath{Planar.Research.MonogenicOrder} directly reuses Mathlib's
\leanpath{Algebra.adjoin}, \leanpath{Algebra.adjoin.powerBasis'},
\leanpath{minpoly.equivAdjoin}, and \leanpath{conductor} for the monogenic
comparison order.  Tate reconstruction, the conductor--codifferent identity,
the common embeddings in \(N/T\), and the ideal containment
\((\mathrm{SFTL}_C)\) remain to be formalized.  These prospective objects are
