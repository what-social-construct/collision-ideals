...
of the scheme of pairs with the same image.  Restriction to the diagonal
has kernel \(\Obs(F)\), whose vanishing over \(\C\) characterizes polynomial
automorphisms.  For a counterexample of generic degree three, we find that
the residual factor of the generic collision algebra is the normal closure
\(N\) of \(L/K\), whose Galois group is \(S_3\).

Indeed, generic base change identifies \(K\otimes_BC_F\) with
\(L\otimes_KL\).  For a finite separable extension and a primitive generator
\(\alpha\), the marked-root decomposition gives
\[
  L\otimes_KL\simeq_L L\times D_\alpha,
\]
with tensor multiplication equal to first projection.  In degree three,
\(D_\alpha\) is split in the normal branch and is the normal closure in
the nonnormal branch.  A degree-three map is already nonautomorphic, so on
this stratum the counterexample condition is equivalent to the Keller
condition.  Classical Galois rigidity excludes the normal branch and yields
the stated \(S_3\)-description.  The marked-root decomposition itself is
Keller-free.  No existence assertion is made, and counterexamples of other
generic degrees are not classified.
\end{abstract}
