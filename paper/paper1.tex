...
\(F\colon\A^n_{\C}\to\A^n_{\C}\), let
\[
  S=\C[x_1,\dots,x_n,y_1,\dots,y_n],
  \qquad
  I_R(F)=\bigl(F_i(x)-F_i(y)\bigr)_{i=1}^n.
\]
The quotient \(C_F=S/I_R(F)\) is the coordinate ring of the collision
scheme of pairs with the same image.  The image in \(C_F\) of the diagonal
ideal \(I_\Delta=(x_i-y_i)_{i=1}^n\) is the obstruction ideal
\(\Obs(F)\), canonically identified with \(I_\Delta/I_R(F)\).  Equivalently,
\(\Obs(F)=\ker(\bar\mu_F)\), where
\(\bar\mu_F\colon C_F\to\C[x_1,\dots,x_n]\) is diagonal evaluation.
Over \(\C\), vanishing of this ideal characterizes polynomial automorphisms.

We analyze this obstruction in generic degree three.  Put
\(B=\C[F_1,\dots,F_n]\), \(K=\Frac(B)\), and
\(L=\C(x_1,\dots,x_n)\).  Generic base change identifies the generic
collision algebra with \(L\otimes_KL\).  For a chosen primitive generator
\(\alpha\), the marked-root decomposition gives a quadratic residual
\(L\)-algebra \(D_\alpha\) and equivalences
\[
  K\otimes_BC_F\simeq_K L\otimes_KL,
  \qquad
  L\otimes_KL\simeq_L L\times D_\alpha,
\]
with tensor multiplication equal to first projection.
This decomposition applies to any finite separable generic extension and
requires no Keller hypothesis.  The Keller condition, though unnecessary for
the decomposition, makes \(F\) \'etale and generically finite, so every
possible counterexample lies in this finite separable setting.  Classical
Galois rigidity excludes generic degrees one and two; generic degree three is
therefore the first possible counterexample degree.  Specialize now to
\(n=3\).  Since \([L:K]=3\), the map \(F\) cannot be a polynomial
automorphism.  Thus, on the generic-degree-three locus,
\[
  F\text{ is a counterexample to }JC(3)
  \quad\Longleftrightarrow\quad
  F\text{ is Keller}.
\]
The residual algebra is split in the normal cubic branch and connected in
the nonnormal branch.  Classical Galois rigidity excludes the normal branch
from the counterexample locus.  Consequently, if this locus is nonempty,
then, with \(N\) denoting the normal closure of \(L/K\),
\[
  ...
  \Gal(N/K)\simeq S_3.
\]
Thus the residual generic collision factor of every such counterexample is
the normal closure \(N\), whose Galois group is \(S_3\).  No existence
assertion is made.
\end{abstract}
...
Let \(F\colon\A^n_{\C}\to\A^n_{\C}\) be a polynomial self-map.  Write
\[
  R_F
  :=\A^n_{\C}\times_{\A^n_{\C}}\A^n_{\C},
\]
where both maps to the middle copy are \(F\).  This is the collision scheme:
its points are ordered pairs with the same image under \(F\), and the
diagonal defines a canonical closed subscheme of \(R_F\).  We ask how the
remaining generic factors are organized around it.

Our end goal is to describe the generic-degree-three counterexample locus
for the complex three-dimensional Jacobian conjecture.  Degree three
already rules out polynomial automorphy, so within this locus the
counterexample condition is equivalent to the Keller condition.  Classical
Galois rigidity then eliminates the normal cubic branch.  Every possible
counterexample in this degree class is therefore nonnormal; its quadratic
residual factor is the normal closure with Galois group \(S_3\).  This is
the conclusion toward which the paper is organized.
...
factor and the residual conjugate algebra as the second.  These finiteness
and separability hypotheses are automatic for Keller maps.  Galois rigidity
then excludes degrees one and two for a counterexample, making generic degree
three the first case to analyze.
...
split in the normal branch and connected in the nonnormal branch.  The
degree assumption already makes \(F\) nonautomorphic, so \(F\) is a
counterexample to \(JC(3)\) if and only if it is Keller.  On this
counterexample locus, classical Galois rigidity excludes the split branch.
Thus \(D_\alpha\) is connected, it is the normal closure \(N\), and
\[
  L\otimes_KL\simeq L\times N,
  \qquad
  \Gal(N/K)\simeq S_3.
\]
Under the chosen power-basis presentation, the canonical diagonal factor
corresponds to \(\alpha\), while the residual factor supplies a second root;
in a cubic extension those two roots determine the third.  The
faithful action of \(G=\Gal(N/K)\) on the three cosets of
\(H=\Gal(N/L)\) gives the permutation representation; the degree count
identifies this Galois group with \(S_3\).
The nonzero residual factor is the collision-theoretic witness of
nonautomorphy.  We do not assert that the generic-degree-three
counterexample locus is nonempty, or that every possible counterexample
has generic degree three.
...
two cubic residual branches and shows that the counterexample locus can
occur only in the nonnormal branch, whose normal closure has Galois group
\(S_3\).  Section
...
\leanpath{CollisionIdeals/General/Keller/Interfaces.lean}, and
...
\(H\neq1\) as explicit inputs.  It proves obstruction nonvanishing before
...
\ref{cor:cubic-counterexample-locus}; the use of classical Galois
